...
// No include guards here, this file may be included many times in different
// namespaces i.e. BACKUP RAM Backup/Restore functions
...
PACK(struct MixData {
  uint16_t destCh : 5;
  int16_t srcRaw
      : 10 CUST(r_mixSrcRawEx, w_mixSrcRawEx);  // srcRaw=0 means not used
...
PACK(struct CustomFunctionData {
  ...
  PACK(union {
    NOBACKUP(PACK(struct { char name[LEN_FUNCTION_NAME]; }) play);
  ...
  bool isEmpty() const { return swtch == 0; }
});
...
PACK(struct LineDataSource { source_t val CUST(r_mixSrcRaw, w_mixSrcRaw); });
PACK(struct FrSkyLineData { LineDataSource sources[NUM_LINE_ITEMS]; });
#else
// This here is the real structure used at run-time
PACK(struct FrSkyLineData { source_t sources[NUM_LINE_ITEMS]; });
#endif
...
PACK(struct VarioData {
  uint8_t source
      : 7 CUST(r_tele_sensor, w_tele_sensor);  // telemetry sensor idx + 1
...
PACK(struct TelemetrySensor {
  ...
  uint8_t type : 1 ENUM(TelemetrySensorType);  // 0=custom / 1=calculated
                                               // user can choose what unit to
                                               // display each value in
  ...
  union {
    ...
    NOBACKUP(PACK(struct { int8_t sources[4]; }) calc);
    NOBACKUP(PACK(struct {
      uint8_t source;
  ...
  NOBACKUP(
      ...
      void init(uint16_t id); bool isAvailable() const;
      int32_t getValue(int32_t value, uint8_t unit, uint8_t prec) const;
      bool isConfigurable() const; bool isPrecConfigurable() const;
      int32_t getPrecMultiplier() const; int32_t getPrecDivisor() const;
...
PACK(struct ModuleData {
  ...
  union {
    ...
    NOBACKUP(struct {
      ...
      uint8_t spare2 : 1 SKIP;
      int8_t refreshRate;  // definition as framelength for ppm (* 5 + 225 =
                           // time in 1/10 ms)
    } sbus);
    NOBACKUP(struct {
  ...
  NOBACKUP(inline uint8_t getChannelsCount()
               const { return channelsCount + 8; })
});
...
#define CUSTOM_SCREENS_DATA                                     \
  uint8_t screensType                                           \
      SKIP; /* 2bits per screen (None/Gauges/Numbers/Script) */ \
...
#define TOPBAR_DATA
    ...
    defined(PCBPL18) || defined(PCBST16)
#define SCRIPT_DATA NOBACKUP(ScriptData scriptsData[MAX_SCRIPTS]);
#else
#define SCRIPT_DATA
#endif
...
struct RGBLedColor {
  ...
  uint32_t getColor() { return ((r << 16) + (g << 8) + b); }
...
PACK(struct USBJoystickChData {
  ...
  NOBACKUP(
      uint8_t btnCount() {
        // Use one less joystick button for 2POS and 3POS switches for Companion
        // mode
        if ((param == USBJOYS_BTN_MODE_COMPANION) && (switch_npos > 0) &&
            (switch_npos < 3))
          return switch_npos;
...
#if defined(BUZZER)
#define BUZZER_FIELD \
  int8_t buzzerMode : 2  // -2=quiet, -1=only alarms, 0=no keys, 1=all (only
                         // used on AVR radios without audio hardware)
#else
...
PACK(struct RadioData {
  ...
  int8_t timezoneMinutes
      : 3;  // -3 to +3 ==> (-45 to 45 minutes in 15 minute increments)
  ...
  NOBACKUP(int8_t beepMode : 2 ENUM(BeeperMode)
               CUST(r_beeperMode, w_beeperMode));
  ...
  NOBACKUP(uint32_t serialPort ARRAY(SERIAL_CONF_BITS_PER_PORT,
                                     struct_serialConfig, nullptr));
...
#if defined(COLORLCD)
  NOBACKUP(uint8_t labelSingleSelect
           : 1);  // 0 = multi-select, 1 = single select labels
  NOBACKUP(uint8_t labelMultiMode
           : 1);  // 0 = match all labels (AND), 1 = match any labels (OR)
  NOBACKUP(uint8_t favMultiMode
           : 1);  // 0 = match all (AND), 1 = match any (OR)
  ...
  NOBACKUP(uint8_t throttleMode : 1);
